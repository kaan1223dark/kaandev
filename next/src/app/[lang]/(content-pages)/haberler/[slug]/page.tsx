...
export default async function NewsPost(props: NewsPostProps) {
    ...
    let pageData = null as APIResponseCollection<'api::news-single.news-single'> | null;

    if (includeDrafts) {
        const manualUrl = getStrapiUrl(`${baseUrl}${params.slug}${baseUrl.includes('?') ? '&' : '?'}publicationState=preview&locale=tr`);
        const res = await fetch(manualUrl, {
            headers: { Authorization: `Bearer ${process.env.STRAPI_API_KEY}` },
            next: { tags: ['news-single'] },
        });

        if (!res.ok) {
            try {
                const text = await res.text();
                logger.error('[news-post] preview fetch failed', { url: manualUrl, status: res.status, body: text });
            } catch (e) {
                logger.error('[news-post] preview fetch failed and response body could not be read', { url: manualUrl, status: res.status });
            }

            try {
                pageData = await getStrapiData<APIResponseCollection<'api::news-single.news-single'>>('tr', `${baseUrl}${params.slug}`, ['news-single']);
            } catch (err) {
                logger.error('[news-post] fallback live fetch also failed', err);
                pageData = null;
            }
        } else {
            const data = await res.json();
            pageData = data as APIResponseCollection<'api::news-single.news-single'>;
        }
    } else {
        pageData = await getStrapiData<APIResponseCollection<'api::news-single.news-single'>>('tr', `${baseUrl}${params.slug}`, ['news-single']);
    }
